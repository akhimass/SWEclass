...
def read_one_sandwich(sandwich_id: int, db: Session = Depends(get_db)):
    sandwich = sandwiches.read_one(db, item_id=sandwich_id)
    if sandwich is None:
...
def update_sandwich(sandwich_id: int, sandwich: schemas.SandwichUpdate, db: Session = Depends(get_db)):
    return sandwiches.update(db=db, item_id=sandwich_id, sandwich=sandwich)

@app.delete("/sandwiches/{sandwich_id}", tags=["Sandwiches"])
def delete_sandwich(sandwich_id: int, db: Session = Depends(get_db)):
    return sandwiches.delete(db=db, item_id=sandwich_id)
...
def read_one_resource(resource_id: int, db: Session = Depends(get_db)):
    resource = resources.read_one(db, item_id=resource_id)
    if resource is None:
...
def update_resource(resource_id: int, resource: schemas.ResourceUpdate, db: Session = Depends(get_db)):
    return resources.update(db=db, item_id=resource_id, resource=resource)

@app.delete("/resources/{resource_id}", tags=["Resources"])
def delete_resource(resource_id: int, db: Session = Depends(get_db)):
    return resources.delete(db=db, item_id=resource_id)
...
def read_one_recipe(recipe_id: int, db: Session = Depends(get_db)):
    recipe = recipes.read_one(db, item_id=recipe_id)
    if recipe is None:
...
def update_recipe(recipe_id: int, recipe: schemas.RecipeUpdate, db: Session = Depends(get_db)):
    return recipes.update(db=db, item_id=recipe_id, recipe=recipe)

@app.delete("/recipes/{recipe_id}", tags=["Recipes"])
def delete_recipe(recipe_id: int, db: Session = Depends(get_db)):
    return recipes.delete(db=db, item_id=recipe_id)
...
def read_one_order_detail(detail_id: int, db: Session = Depends(get_db)):
    detail = order_details.read_one(db, item_id=detail_id)
    if detail is None:
...
def update_order_detail(detail_id: int, detail: schemas.OrderDetailUpdate, db: Session = Depends(get_db)):
    return order_details.update(db=db, item_id=detail_id, detail=detail)

@app.delete("/order_details/{detail_id}", tags=["Order Details"])
def delete_order_detail(detail_id: int, db: Session = Depends(get_db)):
    return order_details.delete(db=db, item_id=detail_id)
